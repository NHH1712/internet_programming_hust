...
int main(int argc, char* argv[]) {
    ...
    printf("Đang lắng nghe kết nối... \n");

    while (1) {
        // Xóa tập hợp readfds và thêm server socket vào tập hợp
        FD_ZERO(&readfds);
        FD_SET(serverSocket, &readfds);
        max_sd = serverSocket;

        // Thêm các client socket vào tập hợp
        for (i = 0; i < maxClients; i++) {
            sd = clientSockets[i];
            if (sd > 0)
                FD_SET(sd, &readfds);
        ...
        // Kiểm tra các sự kiện trên các client socket
        for (i = 0; i < maxClients; i++) {
            sd = clientSockets[i];
